  ...
  char humidity_str[10];
	...
	while(1){
		if(HAL_GetTick()-last>1500){
			...
			sprintf(humidity_str, "%.0f", Humidity);
			lcd_put_cur(0,0);
			lcd_send_string("T:");
			lcd_put_cur(1,0);
			lcd_send_string("H:");	
...
void Read_MQ(void){
  ...
  // Read ADC value
  MQ6 = HAL_ADC_GetValue(&hadc1);
  sprintf(MQ6_str, "%d", MQ6);
  // Stop ADC conversion
  HAL_ADC_Stop(&hadc1);
	lcd_put_cur(0,10);
	lcd_send_string("GAS:");
	lcd_put_cur(1,10);
	lcd_send_string(MQ6_str);
	  if (MQ6 > ValueTOP) {
    // N?u MQ6 l?n hon ValueTOP, thi?t l?p chân PA7 là HIGH
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
  } else {
    // Ngu?c l?i, thi?t l?p chân PA7 là LOW
